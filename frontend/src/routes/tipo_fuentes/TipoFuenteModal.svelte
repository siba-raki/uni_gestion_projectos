<script>
	...
	export let tipoFuente;
	export let onClose;
	export let getTipoFuentes;

	async function editTipoFuente() {
		try {
			await updateData(`tipo_fuentes/${tipoFuente.id}`, tipoFuente);
			onClose();
			getTipoFuentes();
		} catch (error) {
...
>
	<div class="bg-white rounded-lg shadow overflow-hidden max-w-lg mx-auto p-8">
		<form on:submit|preventDefault={editTipoFuente}>
			<div class="flex items-center space-x-4">
				<div class="flex items-center">
					...
					<input
						type="text"
						id="descripcion"
						bind:value={tipoFuente.descripcion}
				...
				<div class="flex items-center">
					...
					<input
						type="checkbox"
						id="activo"
						bind:checked={tipoFuente.activo}
