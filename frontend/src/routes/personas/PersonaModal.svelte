<script>
    ...
    async function editPersona() {
        try {
            await updateData(`personas/${persona.id}`, persona);
            onClose();
			getPersonas();
        } catch (error) {
...
>
	<div class="bg-white rounded-lg shadow overflow-hidden max-w-lg mx-auto p-8">
		<form on:submit|preventDefault={editPersona}>
			<div class="flex items-center space-x-4">
				<div class="flex items-center">
					<label for="nombre" class="block text-gray-700 text-sm font-bold mr-2">
						Nombre:
					</label>
					<input
						type="text"
                ...
                <div class="flex items-center">
					<label for="apellido" class="block text-gray-700 text-sm font-bold mr-2">
						Apellido:
					</label>
					<input
						type="text"
                ...
                <div class="flex items-center">
					...
					<input
						type="date"
						id="fecha_nacimiento"
						bind:value={persona.fecha_nacimiento}
						class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
					/>
				</div>
				<div class="flex items-center">
