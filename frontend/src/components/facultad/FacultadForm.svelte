<script>
    ...
    async function submitForm() {
        try {
            await postData(`facultades`, {descripcion, activo});
			getFacultades();
        } catch (error) {
...
<div class="bg-white text-custom-paleblue p-4">
    <form on:submit|preventDefault={submitForm} class="flex items-center justify-between">
        <div class="flex items-center space-x-4">
            ...
            <button type="submit" class="bg-custom-blue hover:bg-custom-lightblue text-white font-bold py-2 px-4 rounded">Agregar Facultad</button>
        </div>
    </form>
</div>
